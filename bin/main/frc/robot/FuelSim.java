...
import frc.robot.subsystems.ExtendedTranslation2d;
...
public class FuelSim {
    ...
    private static final ExtendedTranslation3d GRAVITY = new ExtendedTranslation3d(0, 0, -9.81); // m/s^2
    ...
    private static final ExtendedTranslation3d[] FIELD_XZ_LINE_STARTS = {
        new ExtendedTranslation3d(0, 0, 0),
        new ExtendedTranslation3d(3.96, 1.57, 0),
        new ExtendedTranslation3d(3.96, FIELD_WIDTH / 2 + 0.60, 0),
        new ExtendedTranslation3d(4.61, 1.57, 0.165),
        new ExtendedTranslation3d(4.61, FIELD_WIDTH / 2 + 0.60, 0.165),
        new ExtendedTranslation3d(FIELD_LENGTH - 5.18, 1.57, 0),
        new ExtendedTranslation3d(FIELD_LENGTH - 5.18, FIELD_WIDTH / 2 + 0.60, 0),
        new ExtendedTranslation3d(FIELD_LENGTH - 4.61, 1.57, 0.165),
        new ExtendedTranslation3d(FIELD_LENGTH - 4.61, FIELD_WIDTH / 2 + 0.60, 0.165)
    };

    private static final ExtendedTranslation3d[] FIELD_XZ_LINE_ENDS = {
        new ExtendedTranslation3d(FIELD_LENGTH, FIELD_WIDTH, 0),
        new ExtendedTranslation3d(4.61, FIELD_WIDTH / 2 - 0.60, 0.165),
        new ExtendedTranslation3d(4.61, FIELD_WIDTH - 1.57, 0.165),
        new ExtendedTranslation3d(5.18, FIELD_WIDTH / 2 - 0.60, 0),
        new ExtendedTranslation3d(5.18, FIELD_WIDTH - 1.57, 0),
        new ExtendedTranslation3d(FIELD_LENGTH - 4.61, FIELD_WIDTH / 2 - 0.60, 0.165),
        new ExtendedTranslation3d(FIELD_LENGTH - 4.61, FIELD_WIDTH - 1.57, 0.165),
        new ExtendedTranslation3d(FIELD_LENGTH - 3.96, FIELD_WIDTH / 2 - 0.60, 0),
        new ExtendedTranslation3d(FIELD_LENGTH - 3.96, FIELD_WIDTH - 1.57, 0)
    };

    private class Fuel {
        private ExtendedTranslation3d pos;
        private ExtendedTranslation3d vel;

        private Fuel(ExtendedTranslation3d pos, ExtendedTranslation3d vel) {
            this.pos = pos;
            this.vel = vel;
        }

        private Fuel(ExtendedTranslation3d pos) {
            this(pos, new ExtendedTranslation3d(0, 0, 0)); // Provide appropriate default values for the constructor
        }
        ...
        private void update() {
            ...
            if (Math.abs(vel.getZ()) < 0.05 && pos.getZ() <= FUEL_RADIUS + 0.03) {
                vel = new ExtendedTranslation3d(vel.getX(), vel.getY(), 0);
                vel = vel.times(1 - FRICTION * PERIOD / subticks);
                // pos = new ExtendedTranslation3d(pos.getX(), pos.getY(), FUEL_RADIUS);
            }
            handleFieldCollisions();
        }

        private void handleXZLineCollision(ExtendedTranslation3d lineStart, ExtendedTranslation3d lineEnd) {
            if (pos.getY() < lineStart.getY() || pos.getY() > lineEnd.getY()) return; // not within y range
            // Convert into 2D
            ExtendedTranslation2d start2d = new ExtendedTranslation2d(lineStart.getX(), lineStart.getZ());
            ExtendedTranslation2d end2d = new ExtendedTranslation2d(lineEnd.getX(), lineEnd.getZ());
            ExtendedTranslation2d pos2d = new ExtendedTranslation2d(pos.getX(), pos.getZ());
            ExtendedTranslation2d lineVec = new ExtendedTranslation2d(end2d.getX() - start2d.getX(), end2d.getY() - start2d.getY());

            // Get closest point on line
            // Line 95 Fix: Using times() for division and ensuring proper 3D vector construction
            ExtendedTranslation3d normal = new ExtendedTranslation3d(-lineVec.getY(), 0, lineVec.getX()).times(1.0 / lineVec.getNorm());
            // 1. Get the raw coordinates of the vector from start to fuel
            double dx = pos2d.getX() - start2d.getX();
            double dy = pos2d.getY() - start2d.getY();

            // 2. Calculate the dot product and square norm manually
            double dotProduct = dx * lineVec.getX() + dy * lineVec.getY();
            double squareNorm = Math.pow(lineVec.getX(), 2) + Math.pow(lineVec.getY(), 2);

            // 3. Project onto the line
            ExtendedTranslation2d projected = (ExtendedTranslation2d) start2d.plus(lineVec.times(dotProduct / squareNorm));

            if (projected.getDistance(start2d) + normal.getDistance(new ExtendedTranslation3d(end2d.getX(), 0, end2d.getY())) > lineVec.getNorm())
        ...
        private void handleFieldCollisions() {
            ...
            if (pos.getX() < FUEL_RADIUS && vel.getX() < 0) {
                pos = pos.plus(new ExtendedTranslation3d(FUEL_RADIUS - pos.getX(), 0, 0));
                vel = vel.plus(new ExtendedTranslation3d(-(1 + FIELD_COR) * vel.getX(), 0, 0));
            } else if (pos.getX() > FIELD_LENGTH - FUEL_RADIUS && vel.getX() > 0) {
                pos = pos.plus(new ExtendedTranslation3d(FIELD_LENGTH - FUEL_RADIUS - pos.getX(), 0, 0));
                vel = vel.plus(new ExtendedTranslation3d(-(1 + FIELD_COR) * vel.getX(), 0, 0));
            }

            if (pos.getY() < FUEL_RADIUS && vel.getY() < 0) {
                pos = pos.plus(new ExtendedTranslation3d(0, FUEL_RADIUS - pos.getY(), 0));
                vel = vel.plus(new ExtendedTranslation3d(0, -(1 + FIELD_COR) * vel.getY(), 0));
            } else if (pos.getY() > FIELD_WIDTH - FUEL_RADIUS && vel.getY() > 0) {
                pos = pos.plus(new ExtendedTranslation3d(0, FIELD_WIDTH - FUEL_RADIUS - pos.getY(), 0));
                vel = vel.plus(new ExtendedTranslation3d(0, -(1 + FIELD_COR) * vel.getY(), 0));
            }
        ...
        private void handleHubCollisions(Hub hub) {
            ...
            if (netCollision != 0) {
                pos = pos.plus(new ExtendedTranslation3d(netCollision, 0, 0));
                vel = new ExtendedTranslation3d(-vel.getX() * NET_COR, vel.getY() * NET_COR, vel.getZ());
            }
        }

        private void addImpulse(ExtendedTranslation3d impulse) {
            vel = vel.plus(impulse);
        }
    }

    private static void handleFuelCollision(Fuel a, Fuel b) {
        ExtendedTranslation3d normal = a.pos.minus(b.pos);
        double distance = normal.getNorm();
        if (distance == 0) {
            normal = new ExtendedTranslation3d(1, 0, 0);
            distance = 1;
        }
    ...
    public void spawnStartingFuel() {
        ...
        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 6; j++) {
                fuels.add(new Fuel(center.plus(new ExtendedTranslation3d(0.076 + 0.152 * j, 0.0254 + 0.076 + 0.152 * i,
        0))));
                fuels.add(new Fuel(center.plus(new ExtendedTranslation3d(-0.076 - 0.152 * j, 0.0254 + 0.076 + 0.152 * i,
        0))));
                fuels.add(new Fuel(center.plus(new ExtendedTranslation3d(0.076 + 0.152 * j, -0.0254 - 0.076 - 0.152 * i,
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 4; j++) {
                fuels.add(new Fuel(new ExtendedTranslation3d(0.076 + 0.152 * j, 5.95 + 0.076 + 0.152 * i, FUEL_RADIUS)));
                fuels.add(new Fuel(new ExtendedTranslation3d(0.076 + 0.152 * j, 5.95 - 0.076 - 0.152 * i, FUEL_RADIUS)));
                fuels.add(new Fuel(
                        new ExtendedTranslation3d(FIELD_LENGTH - 0.076 - 0.152 * j, 2.09 + 0.076 + 0.152 * i, FUEL_RADIUS)));
                fuels.add(new Fuel(
                        new ExtendedTranslation3d(FIELD_LENGTH - 0.076 - 0.152 * j, 2.09 - 0.076 - 0.152 * i, FUEL_RADIUS)));
            }
        }
    ...
    private void handleRobotCollision(Fuel fuel, Pose2d robot, ExtendedTranslation2d robotVel) {
        ...
        ExtendedTranslation2d posOffset;
        ...
        if ((distanceToBottom >= distanceToTop
            ...
            posOffset = new ExtendedTranslation2d(distanceToBottom, 0);
        } else if ((distanceToTop >= distanceToBottom
                        && distanceToTop >= distanceToRight
                        && distanceToTop >= distanceToLeft)) {
            posOffset = new ExtendedTranslation2d(-distanceToTop, 0);
        } else if ((distanceToRight >= distanceToBottom
                        && distanceToRight >= distanceToTop
                        && distanceToRight >= distanceToLeft)) {
            posOffset = new ExtendedTranslation2d(0, distanceToRight);
        } else {
            posOffset = new ExtendedTranslation2d(0, -distanceToLeft);
        }

        posOffset = new ExtendedTranslation2d(posOffset.rotateBy(robot.getRotation()));
        fuel.pos = fuel.pos.plus(new Translation3d(posOffset));
        ExtendedTranslation2d normal = new ExtendedTranslation2d(posOffset.div(posOffset.getNorm()));
        Translation2d fuelVel2d = fuel.vel.toTranslation2d();

        // 2. Manually calculate the dot product: (v1.x * v2.x) + (v1.y * v2.y)
        double velocityDotNormal = (fuelVel2d.getX() * normal.getX()) + (fuelVel2d.getY() * normal.getY());
                
        if (new ExtendedTranslation2d(fuel.vel.toTranslation2d()).dot(normal) < 0)
            fuel.addImpulse(
                new ExtendedTranslation3d(normal.times(-velocityDotNormal * (1 + ROBOT_COR))));
        if (robotVel.dot(normal) > 0) {
            Translation2d impulse2d = normal.times(robotVel.dot(normal));
            fuel.addImpulse(new ExtendedTranslation3d(impulse2d.getX(), impulse2d.getY(), 0));
        }
    }
    ...
    private void handleRobotCollisions(ArrayList<Fuel> fuels) {
        ...
        ExtendedTranslation2d robotVel = new ExtendedTranslation2d(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond);
    ...
    public static class Hub {
        public static final Hub BLUE_HUB =
                new Hub(new ExtendedTranslation2d(4.61, FIELD_WIDTH / 2), new ExtendedTranslation3d(5.3, FIELD_WIDTH / 2, 0.89), 1);
        public static final Hub RED_HUB = new Hub(
                new ExtendedTranslation2d(FIELD_LENGTH - 4.61, FIELD_WIDTH / 2),
                new ExtendedTranslation3d(FIELD_LENGTH - 5.3, FIELD_WIDTH / 2, 0.89),
                -1);
        ...
        private final ExtendedTranslation2d center;
        private final ExtendedTranslation3d exit;
        private final int exitVelXMult;

        private int score = 0;

        private Hub(ExtendedTranslation2d center, ExtendedTranslation3d exit, int exitVelXMult) {
            this.center = center;
            this.exit = exit;
        ...
        private boolean didFuelScore(Fuel fuel) {
            // Line 376 Fix: Compare the 2D center to the fuel's 2D position for the radius check
            return fuel.pos.toTranslation2d().getDistance(center) <= ENTRY_RADIUS
                && fuel.pos.getZ() <= ENTRY_HEIGHT
                && fuel.pos.getZ() > (fuel.pos.getZ() - (fuel.vel.getZ() * PERIOD / subticks)); 
            // Note: Logic simplified to check if it crossed the ENTRY_HEIGHT plane downwards   
        }

        private ExtendedTranslation3d getDispersalVelocity() {
            return new ExtendedTranslation3d(exitVelXMult * (Math.random() + 0.1) * 1.5, Math.random() * 2 - 1, 0);
        }
